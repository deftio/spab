 ...
 *   node --experimental-sqlite bench/db.js import
 *   node --experimental-sqlite bench/db.js runs
 *   node --experimental-sqlite bench/db.js query
 *   node --experimental-sqlite bench/db.js sql "SELECT ..."
 */
'use strict';
...
try { DatabaseSync = require('node:sqlite').DatabaseSync; }
  ...
  console.error('node:sqlite unavailable. Run with:  node --experimental-sqlite bench/db.js <cmd>');
  console.error('(needs Node >= 22.5). The JSONL logs are unaffected.');
  process.exit(1);
}

var LOG_DIR = path.join(__dirname, 'runs');
