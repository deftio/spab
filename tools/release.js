...
// No agent/session identifiers in the history being pushed.
//
// A session URL in a commit message is a permanent, resolvable pointer from public
// source history to a private conversation, and once pushed there is no practical
// way to remove it: the commit object stays fetchable by SHA, the PR timeline keeps
// the record, and only GitHub Support can purge either. A revert does not help — it
// adds a commit, it does not remove one. So this is checked BEFORE the push, which
// is the last moment it can still be fixed with `git commit --amend`.
const FORBIDDEN_IN_MESSAGES = [
  { re: /claude[-_ ]?session/i, what: 'a Claude session identifier' },
  { re: /claude\.ai\/code\/session/i, what: 'a Claude session URL' },
  { re: /chatgpt\.com\/(c|share)\//i, what: 'a ChatGPT conversation URL' }
];

const newCommits = c.tryQuiet('git log origin/main..HEAD --format=%H');
if (newCommits) {
  const offenders = [];
  for (const sha of newCommits.split('\n').filter(Boolean)) {
    const msg = c.tryQuiet('git log -1 --format=%B ' + sha) || '';
    for (const rule of FORBIDDEN_IN_MESSAGES) {
      if (rule.re.test(msg)) offenders.push('    ' + sha.slice(0, 12) + ' contains ' + rule.what);
    }
  }
  if (offenders.length > 0) {
    c.fail(
      'Commit message(s) contain a conversation identifier:\n' +
      offenders.join('\n') + '\n' +
      '  These must never reach the repository — they are permanent once pushed.\n' +
      '  Fix the most recent with:  git commit --amend\n' +
      '  Older ones:                git rebase -i origin/main'
    );
  }
  c.ok(newCommits.split('\n').filter(Boolean).length + ' commit message(s) clean');
}
